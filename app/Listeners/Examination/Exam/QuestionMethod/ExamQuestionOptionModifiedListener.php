...
class ExamQuestionOptionModifiedListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  ExamQuestionOptionModifiedEvent  $event
     * @return void
     */
    public function handle(ExamQuestionOptionModifiedEvent $event)
    {
        //注释修改后，添加到用户动态，写入协作计划事件，写入试卷动态
        $option = $event->examQuestionOption;
        if($option->sort == '1'){
          $sort = 'A';  
        }elseif($option->sort == '2'){
          $sort = 'B';  
        }elseif($option->sort == '3'){
          $sort = 'C';  
        }elseif($option->sort == '4'){
          $sort = 'D';  
        }
        $question = ExamQuestion::find($option->qid);
        ...
        // 添加事件到用户动态
        $behavior = '编辑了试卷第（'.$question->sort.'）题选项'.$sort;
        // 没有为选项设定名称，因此要么截取，要么直接用字符串代替了
        $objectName = substr($option->option,0,30).'...';
        ...
        // 添加事件到著作动态
        $ExamBehavior = '试卷第（'.$question->sort.'）题选项'.$sort.'已经重新编辑：';
