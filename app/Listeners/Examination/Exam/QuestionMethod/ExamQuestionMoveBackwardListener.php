...
class ExamQuestionMoveBackwardListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  ExamQuestionMoveBackwardEvent  $event
     * @return void
     */
    public function handle(ExamQuestionMoveBackwardEvent $event)
    {
        //试题后移后，添加到用户动态，写入协作计划事件，写入试卷动态
        ...
        // 添加事件到用户动态
        $behavior = '后移了试卷第（'.$question->sort.'）题';
        ...
        // 添加事件到著作动态
        $examBehavior = '试卷第（'.$question->sort.'）题已经后移：';
