...
namespace App\Listeners\Examination\Exam\PartStem;
...
class PartStemMoveBackwardListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  PartStemMoveBackwardEvent  $event
     * @return void
     */
    public function handle(PartStemMoveBackwardEvent $event)
    {
        //材料移动后，添加到用户动态，写入协作计划事件，写入试卷动态
        ...
        // 添加事件到用户动态
        $behavior = '前移了原试卷材料（'.$stem->sort.'）';
        // 没有为材料设定名称，因此要么截取，要么直接用字符串代替了
        $objectName = '材料（'.$stem->sort.'）';
        ...
        // 添加事件到著作动态
        $examBehavior = '试卷材料（'.$stem->sort.'）已经前移：';
