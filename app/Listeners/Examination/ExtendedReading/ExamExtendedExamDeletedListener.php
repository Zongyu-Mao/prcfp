...
class ExamExtendedExamDeletedListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  ExamExtendedExamDeletedEvent  $event
     * @return void
     */
    public function handle(ExamExtendedExamDeletedEvent $event)
    {
        ...
        $user = auth('api')->user();
        // 添加协作事件
        ExamCooperationEvent::cooperationEventAdd($cooperation->id,$user->id,$user->username,'原向试卷的延伸阅读《'.$extended->title.'》已经删除。');
        // 添加事件到用户动态
        $behavior = '删除了延伸阅读：';
