...
use App\Home\Examination\Exam\Extended\ExamExtendedExam;
...
class ExamExtendedExamAddedListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  ExamExtendedExamAddedEvent  $event
     * @return void
     */
    public function handle(ExamExtendedExamAddedEvent $event)
    {
        // 得到延伸、被延伸试卷,写入用户动态和协作事件，不需要通知
        $extend = $event->examExtendedExam;
        $exam = Exam::find($extend->exam_id);
        $extended = Exam::find($extend->extended_id);
        //该处所要处理的是延伸阅读添加后，通知本词条相关用户该信息
        $cooperation = ExamCooperation::find($exam->cooperation_id);
        $user = auth('api')->user();
        // 添加协作事件
        ExamCooperationEvent::cooperationEventAdd($cooperation->id,$user->id,$user->username,'添加了试卷延伸阅读《'.$extended->title.'》。');
        // 添加事件到用户动态
        $behavior = '添加了延伸阅读：';
