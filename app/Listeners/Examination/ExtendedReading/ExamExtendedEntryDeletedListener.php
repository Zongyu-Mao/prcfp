...
class ExamExtendedEntryDeletedListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  ExamExtendedEntryDeletedEvent  $event
     * @return void
     */
    public function handle(ExamExtendedEntryDeletedEvent $event)
    {
        // 得到延伸著作、被延伸词条,写入用户动态和协作事件，不需要通知
        $extend = $event->examExtendedEntry;
        $exam = Exam::find($extend->exam_id);
        $extended = Entry::find($extend->extended_id);
        $cooperation = ExamCooperation::find($exam->cooperation_id);
        $user = auth('api')->user();
        // 添加协作事件
        ExamCooperationEvent::cooperationEventAdd($cooperation->id,$user->id,$user->username,'原向词条的延伸阅读《'.$extended->title.'》已经删除。');
        // 添加事件到用户动态
        $behavior = '删除了延伸(词条)阅读：';
