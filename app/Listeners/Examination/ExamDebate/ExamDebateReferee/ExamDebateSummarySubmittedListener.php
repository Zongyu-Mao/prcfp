...
use App\Notifications\Examination\ExamDebate\ExamDebateClear\ExamDebateAutomaticallyClearedNotification;
use App\Notifications\Examination\ExamDebate\ExamDebateClear\ExamDebateAutomaticallyClearedToUserNotification;
...
class ExamDebateSummarySubmittedListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  ExamDebateSummarySubmittedEvent  $event
     * @return void
     */
    public function handle(ExamDebateSummarySubmittedEvent $event)
    {
        ...
        //添加事件到辩论事件表
        ExamDebateEvent::debateEventAdd($debate->id,$debate->referee_id,$debate->referee,'提交攻辩总结。');
        ExamDebateEvent::debateEventAdd($debate->id,$debate->Aauthor_id,$debate->Aauthor,'攻辩已经结束。');
        //计算双方的奖励和裁判工资
        $rewardA = $debate->ARedstars*5 - $debate->ABlackstars*10;
        $rewardB = $debate->BRedstars*5 - $debate->BBlackstars*10;
        $rewardR = $debate->RRedstars*5 - $debate->RBlackstars*10 + 200;
        if($debate->victory == '1'){
            $rewardA += '100';
            // 添加热度记录
            $b_id = 66;
        ...
        Announcement::announcementAdd('3','3','试卷《'.$exam->title.'攻辩<'.$debate->title.'>已经结束[裁判结算]。','/examination/debate/'.$exam->id.'/'.$exam->title.'?type='.$debate->type.'&type_id='.$debate->type_id,$debate->updated_at);
        // 攻辩结束不需要添加到用户动态
        ...
        // 添加热度记录
        $be_id = Behavior::where('sort',63)->first()->id;
        if(ExamTemperatureRecord::where([['exam_id',$exam->id],['user_id',$debate->referee_id],['behavior_id',$be_id]])->count() < 3){
             ExamTemperatureRecord::recordAdd($exam->id,$debate->referee_id,$b_id,$createtime);
        }
        // 发布通知
        // 发送通知给辩论参与人员
        User::find($debate->Aauthor_id)->notify(new ExamDebateAutomaticallyClearedToUserNotification($debate));
        User::find($debate->Bauthor_id)->notify(new ExamDebateAutomaticallyClearedToUserNotification($debate));
        User::find($debate->referee_id)->notify(new ExamDebateAutomaticallyClearedToUserNotification($debate));
        // 获取词条母专业兴趣人员
        ...
        // 发送通知
        Notification::send($usersToNotification, new ExamDebateAutomaticallyClearedNotification($debate));
    }
}
