...
class ExamDebateAutomaticallyClearedListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  ExamDebateAutomaticallyClearedEvent  $event
     * @return void
     */
    public function handle(ExamDebateAutomaticallyClearedEvent $event)
    {
        ...
        Announcement::announcementAdd('3','3','试卷《'.$exam->title.'攻辩<'.$debate->title.'>已经结束[正常结算]。','/examination/debate/'.$exam->id.'/'.$exam->title.'?type='.$debate->type.'&type_id='.$debate->type_id,$debate->updated_at);
        // 攻辩结束不需要添加到用户动态
