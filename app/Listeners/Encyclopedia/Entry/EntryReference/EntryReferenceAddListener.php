...
use App\Notifications\Encyclopedia\Entry\EntryReference\EntryReferenceAddNotification;
...
class EntryReferenceAddListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  EntryReferenceAddEvent  $event
     * @return void
     */
    public function handle(EntryReferenceAddEvent $event)
    {
        //词条参考文献添加后，添加该事件到协作动态，通知协作组成员和词条关注者参考文献新增了
        ...
        // 添加事件到词条动态
        $entryBehavior = '已经添加参考文献';
        EntryDynamic::dynamicAdd($entry->id,$entry->title,$entryBehavior,$objectName,$objectURL,$createtime);
        // 添加协作事件
        EntryCooperationEvent::cooperationEventAdd($cooperation->id,$user->id,$user->username,'添加了参考文献['.$ref->sort.']<'.$ref->title.'>。');
        // 开启对协作组成员和关注词条用户的通知
        ...
        Notification::send($usersToNotification, new EntryReferenceAddNotification($ref));
