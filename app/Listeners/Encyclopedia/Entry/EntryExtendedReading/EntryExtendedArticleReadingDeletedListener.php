...
use App\Notifications\Encyclopedia\Entry\EntryExtendedReading\EntryExtendedArticleReadingDeletedNotification;
use App\Notifications\Encyclopedia\Entry\EntryExtendedReading\EntryExtendedArticleReadingBeenDeletedNotification;
...
class EntryExtendedArticleReadingDeletedListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  EntryExtendedArticleReadingDeletedEvent  $event
     * @return void
     */
    public function handle(EntryExtendedArticleReadingDeletedEvent $event)
    {
        // 得到延伸词条、被延伸著作
        ...
        // 被延伸著作添加热度记录
        $be_id = 13;
        ArticleTemperatureRecord::recordAdd($extended->id,auth('api')->user()->id,$be_id,$createtime);
        //该处所要处理的是延伸阅读添加后，通知本词条相关用户该信息
        $cooperation = EntryCooperation::find($entry->cooperation_id);
        // 添加协作事件
        EntryCooperationEvent::cooperationEventAdd($cooperation->id,auth('api')->user()->id,auth('api')->user()->username,'删除了(著作)延伸阅读《'.$extended->title.'》。');
        ...
        Notification::send($usersToNotification, new EntryExtendedArticleReadingDeletedNotification($ex));

        // 添加事件到用户动态
        $behavior = '删除了(著作)延伸阅读：';
        ...
        Notification::send($usersToNotification, new EntryExtendedArticleReadingBeenDeletedNotification($ex));
    }
}
