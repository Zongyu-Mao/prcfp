...
use App\Notifications\Encyclopedia\Entry\EntryExtendedReading\EntryExtendedExamReadingDeletedNotification;
use App\Notifications\Encyclopedia\Entry\EntryExtendedReading\EntryExtendedExamReadingBeenDeletedNotification;
...
class EntryExtendedExamReadingDeletedListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  EntryExtendedExamReadingDeletedEvent  $event
     * @return void
     */
    public function handle(EntryExtendedExamReadingDeletedEvent $event)
    {
        // 得到延伸词条、被延伸试卷
        ...
        // 被延伸著作添加热度记录
        $be_id = 13;
        ExamTemperatureRecord::recordAdd($extended->id,auth('api')->user()->id,$be_id,$createtime);
        // 添加协作事件
        EntryCooperationEvent::cooperationEventAdd($cooperation->id,auth('api')->user()->id,auth('api')->user()->username,'删除了(试卷)延伸阅读《'.$extended->title.'》。');
        ...
        Notification::send($usersToNotification, new EntryExtendedExamReadingDeletedNotification($ex));

        // 添加事件到用户动态
        $behavior = '删除了(试卷)延伸阅读：';
        ...
        Notification::send($usersToNotification, new EntryExtendedExamReadingBeenDeletedNotification($ex));
    }
}
