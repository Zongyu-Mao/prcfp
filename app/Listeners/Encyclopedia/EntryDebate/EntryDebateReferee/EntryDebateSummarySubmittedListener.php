...
class EntryDebateSummarySubmittedListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  EntryDebateSummarySubmittedEvent  $event
     * @return void
     */
    public function handle(EntryDebateSummarySubmittedEvent $event)
    {
        ...
        //添加事件到辩论事件表
        EntryDebateEvent::debateEventAdd($debate->id,$debate->referee_id,$debate->referee,'提交攻辩总结。');
        ...
        // 添加裁判总结的热度记录
        $be_id = 63;
        if(EntryTemperatureRecord::where([['eid',$entry->id],['user_id',$debate->referee_id],['behavior_id',$be_id]])->count() < 3){
        ...
        // 发布公告，1代表百科，3代表辩论
        Announcement::announcementAdd('1','3','词条《'.$entry->title.'攻辩<'.$debate->title.'>已经结束[裁判结算]。','/encyclopedia/debate/'.$entry->id.'/'.$entry->title.'?type='.$debate->type.'&type_id='.$debate->type_id,$debate->updated_at);
        // 攻辩结束不需要添加到用户动态
        ...
        User::find($debate->referee_id)->notify(new EntryDebateAutomaticallyClearedToUserNotification($debate));
        // 获取词条母专业兴趣人员
