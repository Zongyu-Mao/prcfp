...
use App\Notifications\Encyclopedia\EntryDebate\EntryDebateClear\EntryDebateTimeOutByRefereeClearedNotification;
use App\Notifications\Encyclopedia\EntryDebate\EntryDebateClear\EntryDebateTimeOutByRefereeClearedToUserNotification;
use App\Notifications\Encyclopedia\EntryDebate\EntryDebateClear\EntryDebateTimeOutByRefereeClearedToRefereeNotification;
...
class EntryDebateTimeOutByRefereeClearedListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  EntryDebateTimeOutByRefereeClearedEvent  $event
     * @return void
     */
    public function handle(EntryDebateTimeOutByRefereeClearedEvent $event)
    {
        // 裁判超时，对裁判惩罚
        ...
        //添加事件到辩论事件表
        EntryDebateEvent::debateEventAdd($debate->id,$debate->Aauthor_id,$debate->Aauthor,'攻辩已经结束（裁判结算超时，自动结算）。');
        //计算双方的奖励和裁判工资
        ...
        // 发布公告，1代表百科，3代表辩论
        Announcement::announcementAdd('1','3','词条《'.$entry->title.'攻辩<'.$debate->title.'>已经结束[裁判结算超时，自动结算]。','/encyclopedia/debate/'.$entry->id.'/'.$entry->title.'?type='.$debate->type.'&type_id='.$debate->type_id,$debate->updated_at);
        // 攻辩结束不需要添加到用户动态
        ...
        // 裁判误时
        $be_id = Behavior::where('sort',68)->first()->id;
        EntryTemperatureRecord::recordAdd($entry->id,$debate->referee_id,$be_id,$createtime);
        // 发布通知
        // 发送通知给辩论参与人员
        User::find($debate->Aauthor_id)->notify(new EntryDebateTimeOutByRefereeClearedToUserNotification($debate));
        User::find($debate->Bauthor_id)->notify(new EntryDebateTimeOutByRefereeClearedToUserNotification($debate));
        User::find($debate->referee_id)->notify(new EntryDebateTimeOutByRefereeClearedToRefereeNotification($debate));
        // 获取词条母专业兴趣人员
        ...
        // 发送通知
        Notification::send($usersToNotification, new EntryDebateTimeOutByRefereeClearedNotification($debate));
    }
}
