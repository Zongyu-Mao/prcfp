...
use App\Notifications\Encyclopedia\EntryDebate\EntryDebateClosed\EntryDebateGivenUpNotification;
use App\Notifications\Encyclopedia\EntryDebate\EntryDebateClosed\EntryDebateGivenUpToUserNotification;
use App\Notifications\Encyclopedia\EntryDebate\EntryDebateClosed\EntryDebateGivenUpToRefereeNotification;
...
class EntryDebateGivenUpListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  EntryDebateGivenUpEvent  $event
     * @return void
     */
    public function handle(EntryDebateGivenUpEvent $event)
    {
        //词条的放弃，和关闭一样，要做很多事情
        ...
        if($debate->status == '2'){
            ...
            // 2发布事件和公告
            // 3添加事件到辩论事件表
            EntryDebateEvent::debateEventAdd($debate->id,$debate->Aauthor_id,$debate->Aauthor,'攻辩:<'.$debate->title.'>由于攻方['.$debate->Aauthor.']放弃关闭。');
            // 发表了有效的讨论后，积分和成长值+100
            // User::expAndGrowValue($debate->Aauthor_id,'100','100');
            // 4发布公告，1代表百科，3代表辩论
            Announcement::announcementAdd('1','3','词条《'.$entry->title.'》辩论<'.$debate->title.'>由于攻方放弃已经关闭。','/encyclopedia/debate/'.$entry->id.'/'.$entry->title.'?type='.$debate->type.'&type_id='.$debate->type_id,$debate->updated_at);
            // 5添加事件到用户动态
            ...
            // 发布通知
            // 发送通知给辩论攻方接收人员
            User::find($debate->Bauthor_id)->notify(new EntryDebateGivenUpToUserNotification($debate));
            ...
            // 2发布事件和公告
            // 3添加事件到辩论事件表
            EntryDebateEvent::debateEventAdd($debate->id,$debate->Bauthor_id,$debate->Bauthor,'攻辩:<'.$debate->title.'>由于辩方['.$debate->Bauthor.']放弃关闭。');
            // 发表了有效的讨论后，积分和成长值+100
            // User::expAndGrowValue($debate->Aauthor_id,'100','100');
            // 4发布公告，1代表百科，3代表辩论
            Announcement::announcementAdd('1','3','词条《'.$entry->title.'》辩论<'.$debate->title.'>由于辩方放弃已经关闭。','/encyclopedia/debate/'.$entry->id.'/'.$entry->title.'?type='.$debate->type.'&type_id='.$debate->type_id,$debate->created_at);
            // 5添加事件到用户动态
        ...
        if($debate->type == '1'){
            ...
            EntryReviewEvent::reviewEventAdd($review_id,$debate->Aauthor_id,$debate->Aauthor,'发起的攻辩:['.$debate->title.']已经失败[辩手放弃]。');
            //改变反对状态为攻辩失败
            ...
            EntryDiscussionEvent::discussionEventAdd($debate->eid,$debate->Aauthor_id,$debate->Aauthor,'发起的攻辩:['.$debate->title.']已经失败[辩手放弃]。');
            //改变反对状态为攻辩失败
        ...
        if($debate->referee_id){
            User::find($debate->referee_id)->notify(new EntryDebateGivenUpToRefereeNotification($debate));
        }
        // 获取词条母专业兴趣人员
        ...
        // 发送通知
        Notification::send($usersToNotification, new EntryDebateGivenUpNotification($debate));
    }
}
