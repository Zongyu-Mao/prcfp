...
use App\Notifications\Publication\ArticleDebate\ArticleDebateClosed\ArticleDebateGivenUpNotification;
use App\Notifications\Publication\ArticleDebate\ArticleDebateClosed\ArticleDebateGivenUpToUserNotification;
use App\Notifications\Publication\ArticleDebate\ArticleDebateClosed\ArticleDebateGivenUpToRefereeNotification;
...
class ArticleDebateGivenUpListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  ArticleDebateGivenUpEvent  $event
     * @return void
     */
    public function handle(ArticleDebateGivenUpEvent $event)
    {
        //著作的放弃，和关闭一样，要做很多事情
        ...
        if($debate->status == '2'){
            // status为2，是由于攻方放弃
            // 1、导致攻辩失败方扣除成长值1000，攻方发起又放弃，因此惩罚加倍
            User::expAndGrowValue($debate->Aauthor_id,0,-1000);
            // 2发布事件和公告
            // 3添加事件到辩论事件表
            ArticleDebateEvent::debateEventAdd($debate->id,$debate->Aauthor_id,$debate->Aauthor,'攻辩:<'.$debate->title.'>由于攻方['.$debate->Aauthor.']放弃关闭。');
            // 发表了有效的讨论后，积分和成长值+100
            // User::expAndGrowValue($debate->Aauthor_id,'100','100');
            // 4发布公告，1代表百科，3代表辩论
            Announcement::announcementAdd('2','3','著作《'.$article->title.'》辩论<'.$debate->title.'>由于攻方放弃已经关闭。','/publication/debate/'.$article->id.'/'.$article->title.'?type='.$debate->type.'&type_id='.$debate->type_id,$debate->updated_at);
            // 5添加事件到用户动态
            $behavior = '[攻方]放弃导致结束攻辩：';
            ...
            // 发布通知
            // 发送通知给辩论攻方接收人员
            User::find($debate->Bauthor_id)->notify(new ArticleDebateGivenUpToUserNotification($debate));
        }elseif($debate->status == '3'){
            // status为3，是由于辩方原因导致
            // 1、导致攻辩失败方扣除成长值500
            User::expAndGrowValue($debate->Bauthor_id,0,-500);
            // 2发布事件和公告
            // 3添加事件到辩论事件表
            ArticleDebateEvent::debateEventAdd($debate->id,$debate->Bauthor_id,$debate->Bauthor,'攻辩:<'.$debate->title.'>由于辩方['.$debate->Bauthor.']放弃关闭。');
            // 发表了有效的讨论后，积分和成长值+100
            // User::expAndGrowValue($debate->Aauthor_id,'100','100');
            // 4发布公告，1代表百科2著作，3代表辩论
            Announcement::announcementAdd('2','3','著作《'.$article->title.'》辩论<'.$debate->title.'>由于辩方放弃已经关闭。','/publication/debate/'.$article->id.'/'.$article->title.'?type='.$debate->type.'&type_id='.$debate->type_id,$debate->created_at);
            // 5添加事件到用户动态
            $behavior = '[辩方]放弃导致结束攻辩：';
            $objectName = $debate->title;
            $objectURL = '/publication/debate/'.$article->id.'/'.$article->title.'?type='.$debate->type.'&type_id='.$debate->type_id;
            $fromName = '著作：'.$article->title;
            $fromURL = '/publication/reading/'.$article->id.'/'.$article->title;
        ...
        if($debate->type == '1'){
            ...
            ArticleReviewEvent::reviewEventAdd($review_id,$debate->Aauthor_id,$debate->Aauthor,'发起的攻辩:['.$debate->title.']已经失败[辩手放弃]。');
            //改变反对状态为攻辩失败
            ...
            ArticleDiscussionEvent::discussionEventAdd($debate->aid,$debate->Aauthor_id,$debate->Aauthor,'发起的攻辩:['.$debate->title.']已经失败[辩手放弃]。');
            //改变反对状态为攻辩失败
        ...
        if($debate->referee_id){
            User::find($debate->referee_id)->notify(new ArticleDebateGivenUpToRefereeNotification($debate));
        }
        ...
        // 发送通知
        Notification::send($usersToNotification, new ArticleDebateGivenUpNotification($debate));
    }
}
