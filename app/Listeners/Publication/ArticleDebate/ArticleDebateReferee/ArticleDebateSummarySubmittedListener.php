...
use App\Notifications\Publication\ArticleDebate\ArticleDebateClear\ArticleDebateAutomaticallyClearedNotification;
use App\Notifications\Publication\ArticleDebate\ArticleDebateClear\ArticleDebateAutomaticallyClearedToUserNotification;
...
class ArticleDebateSummarySubmittedListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  ArticleDebateSummarySubmittedEvent  $event
     * @return void
     */
    public function handle(ArticleDebateSummarySubmittedEvent $event)
    {
        ...
        //添加事件到辩论事件表
        ArticleDebateEvent::debateEventAdd($debate->id,$debate->referee_id,$debate->referee,'提交攻辩总结。');
        ArticleDebateEvent::debateEventAdd($debate->id,$debate->Aauthor_id,$debate->Aauthor,'攻辩已经结束。');
        //计算双方的奖励和裁判工资
        ...
        Announcement::announcementAdd('2','3','著作《'.$article->title.'攻辩<'.$debate->title.'>已经结束[裁判结算]。','/publication/debate/'.$article->id.'/'.$article->title.'?type='.$debate->type.'&type_id='.$debate->type_id,$debate->updated_at);
        // 攻辩结束不需要添加到用户动态
        ...
        if(ArticleTemperatureRecord::where([['aid',$exam->id],['user_id',$debate->referee_id],['behavior_id',$be_id]])->count() < 3){
             ArticleTemperatureRecord::recordAdd($debate->aid,$debate->referee_id,$be_id,$createtime);
        }
        // 发布通知
        // 发送通知给辩论参与人员
        User::find($debate->Aauthor_id)->notify(new ArticleDebateAutomaticallyClearedToUserNotification($debate));
        User::find($debate->Bauthor_id)->notify(new ArticleDebateAutomaticallyClearedToUserNotification($debate));
        User::find($debate->referee_id)->notify(new ArticleDebateAutomaticallyClearedToUserNotification($debate));
        // 获取词条母专业兴趣人员
        ...
        // 发送通知
        Notification::send($usersToNotification, new ArticleDebateAutomaticallyClearedNotification($debate));
    }
}
