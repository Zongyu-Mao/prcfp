...
class ArticleDebateAutomaticallyClearedListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  ArticleDebateAutomaticallyClearedEvent  $event
     * @return void
     */
    public function handle(ArticleDebateAutomaticallyClearedEvent $event)
    {
        ...
        Announcement::announcementAdd('2','3','著作《'.$article->title.'攻辩<'.$debate->title.'>已经结束[正常结算]。','/publication/debate/'.$article->id.'/'.$article->title.'?type='.$debate->type.'&type_id='.$debate->type_id,$debate->updated_at);
        // 攻辩结束不需要添加到用户动态
