...
class ArticleExtendedReadingDeletedListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  ArticleExtendedReadingDeletedEvent  $event
     * @return void
     */
    public function handle(ArticleExtendedReadingDeletedEvent $event)
    {
        // 得到延伸著作、被延伸词条,写入用户动态和协作事件，不需要通知
        $article = Article::find($event->articleExtendedEntryReading->aid);
        $extended = Entry::find($event->articleExtendedEntryReading->extended_id);
        $cooperation = ArticleCooperation::find($article->cooperation_id);
        $user = auth('api')->user();
        // 添加协作事件
        ArticleCooperationEvent::cooperationEventAdd($cooperation->id,$user->id,$user->username,'原百科延伸阅读《'.$extended->title.'》已经删除。');
        // 添加事件到用户动态
        $behavior = '删除了延伸阅读：';
        ...
        // 被延伸著作添加热度记录
        $be_id = 13;
        EntryTemperatureRecord::recordAdd($extended->id,$user->id->user()->id,$be_id,$createtime);
    }
}
