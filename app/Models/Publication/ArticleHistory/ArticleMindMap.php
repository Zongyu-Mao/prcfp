...
namespace App\Models\Publication\ArticleHistory;

use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;

class ArticleMindMap extends Model
{
    use HasFactory;

    protected $fillable = [
        'pid','oid','bid','title','type','creator_id','editor_id'
    ];
    // 得到主内容
    public function basicContent(){
        return $this->hasOne('App\Home\Publication\Article','id','oid');
    }
    ...
    protected function elementChild(){
        return $this-> hasMany('App\Models\Publication\ArticleHistory\ArticleMindMap','pid','id');
    }
    ...
    protected function modifyMindMapRecord($id,$bid,$title,$type,$editor_id) {
        $result = ArticleMindMap::where('id',$id)->update([
            'bid'   => $bid,
            'title'   => $title,
            'type'   => $type,
            'editor_id'   => $editor_id
