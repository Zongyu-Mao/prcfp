...
class SurveillanceExamRecord extends Model
{
    use HasFactory;
    ...
    protected function newRecord($user_id,$sid,$status,$stand,$editor_id,$remark) {
        $result = SurveillanceExamRecord::create([
            'user_id'   => $user_id,
            'sid'   => $sid,
            'status'   => $status,
            'stand'   => $stand,
        ...
        event(new SurveillanceExamEvent($result));
        return $result->id;
    }
    //变更 
    protected function recordUpdate($id,$status,$stand) {
        $result = SurveillanceExamRecord::where('id',$id)->update([
            'status'   => $status,
            'stand'   => $stand
        ]);
        event(new SurveillanceExamEvent(SurveillanceExamRecord::find($id)));
        return $result;
    }
}
