...
class SurveillanceExamMark extends Model
{
    use HasFactory;
    ...
    public function records(){
        return $this->hasOne('App\Models\Committee\Surveillance\SurveillanceExamMarkReactRecord','id','mark_id');    
    }

    //创建
    protected function newMark($user_id,$weight,$tcid,$sid,$mark_ids,$dispose_way,$status,$remark) {
        $result = SurveillanceExamMark::create([
            'user_id'   => $user_id,
            'weight'   => $weight,
            'tcid'   => $tcid,
            'sid'   => $sid,
        ...
        event(new MarkExamEvent($result));
        return $result->id;
    }
}
