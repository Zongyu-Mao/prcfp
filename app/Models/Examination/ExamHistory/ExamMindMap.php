...
namespace App\Models\Examination\ExamHistory;

use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;

class ExamMindMap extends Model
{
    use HasFactory;
    ...
    public function basicContent(){
        return $this->hasOne('App\Home\Examination\Exam','id','oid');
    }
    ...
    protected function elementChild(){
        return $this-> hasMany('App\Models\Examination\ExamHistory\ExamMindMap','pid','id');
    }
    ...
    protected function modifyMindMapRecord($id,$bid,$title,$type,$editor_id) {
        $result = ExamMindMap::where('id',$id)->update([
            'bid'   => $bid,
            'title'   => $title,
            'type'   => $type,
            'editor_id'   => $editor_id
