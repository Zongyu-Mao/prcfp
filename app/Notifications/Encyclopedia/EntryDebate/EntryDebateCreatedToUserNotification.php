...
class EntryDebateCreatedToUserNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        $time = $this->entryDebate->created_at->addDays(5);
        // $doer = User::find($this->entryDebate->Aauthor_id);
        return [
            'creator_id'    => $this->entryDebate->Aauthor_id,
            'creator'       => $this->entryDebate->Aauthor,
            'link'          => $link,
            'matter'        => $this->entryDebate->Aauthor.'向你发起攻辩<'.$this->entryDebate->title.'>。你可以在'.$time.'前回复。[来自词条：《'.$entry->title.'》]。'
        ];
    }
