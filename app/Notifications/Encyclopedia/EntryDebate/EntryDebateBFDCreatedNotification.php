...
class EntryDebateBFDCreatedNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        $time = Carbon::parse($this->entryDebate->BFDcreateTime)->addDays(5);
        // $doer = User::find($this->entryDebate->Aauthor_id);
        return [
            'creator_id'    => $this->entryDebate->Bauthor_id,
            'creator'       => $this->entryDebate->Bauthor,
            'link'          => $link,
            'matter'        => '对方['.$this->entryDebate->Bauthor.']已经在你发起的攻辩：<'.$this->entryDebate->title.'>发表辩方的自由辩论。你可以在'.$time.'前回复。[来自词条：《'.$entry->title.'》]。'
        ];
    }
