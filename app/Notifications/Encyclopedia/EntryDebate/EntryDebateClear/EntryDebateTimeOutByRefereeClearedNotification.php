...
class EntryDebateTimeOutByRefereeClearedNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        if($this->entryDebate->victory == '1'){
            $matter = '攻辩<'.$this->entryDebate->title.'>已经结算。[裁判'.$this->entryDebate->referee.']超时，自动结算。攻方['.$this->entryDebate->Aauthor.']胜出。[来自词条：《'.$entry->title.'》]。';
            $creator = $this->entryDebate->Aauthor;
            $creator_id = $this->entryDebate->Aauthor_id;
        }elseif($this->entryDebate->victory == '2'){
            $matter = '攻辩<'.$this->entryDebate->title.'>已经结算。[裁判'.$this->entryDebate->referee.']超时，自动结算。攻方辩方['.$this->entryDebate->Bauthor.']胜出。[来自词条：《'.$entry->title.'》]。';
