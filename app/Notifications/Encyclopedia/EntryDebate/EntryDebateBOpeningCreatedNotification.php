...
class EntryDebateBOpeningCreatedNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        $time = Carbon::parse($this->entryDebate->BOScreateTime)->addDays(5);
        // $doer = User::find($this->entryDebate->Aauthor_id);
        return [
            'creator_id'    => $this->entryDebate->Bauthor_id,
            'creator'       => $this->entryDebate->Bauthor,
            'link'          => $link,
            'matter'        => '对方['.$this->entryDebate->Bauthor.']已经发表辩方开篇陈词，回应你发起的攻辩<'.$this->entryDebate->title.'>。你可以在'.$time.'前回复。[来自词条：《'.$entry->title.'》]。'
        ];
    }
