...
class EntryDebateGivenUpNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        if($this->entryDebate->status == '2'){
            $matter = '攻辩<'.$this->entryDebate->title.'>由于攻方['.$this->entryDebate->Aauthor.']放弃已经关闭。[来自词条：《'.$entry->title.'》]。';
            $creator = $this->entryDebate->Aauthor;
            $creator_id = $this->entryDebate->Aauthor_id;
        }elseif($this->entryDebate->status == '3'){
            $matter = '攻辩<'.$this->entryDebate->title.'>由于辩方['.$this->entryDebate->Bauthor.']放弃已经关闭。[来自词条：《'.$entry->title.'》]。';
