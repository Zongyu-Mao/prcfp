...
class EntryDebateTimeOutClosedToUserNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        if($this->entryDebate->status == '2'){
            $matter = '你作为辩方参与的攻辩<'.$this->entryDebate->title.'>由于攻方['.$this->entryDebate->Aauthor.']回复超时已经关闭。[来自词条：《'.$entry->title.'》]。';
            $creator = $this->entryDebate->Aauthor;
            $creator_id = $this->entryDebate->Aauthor_id;
        }elseif($this->entryDebate->status == '3'){
            $matter = '你发起的攻辩<'.$this->entryDebate->title.'>由于辩方['.$this->entryDebate->Bauthor.']回复超时已经关闭。[来自词条：《'.$entry->title.'》]。';
