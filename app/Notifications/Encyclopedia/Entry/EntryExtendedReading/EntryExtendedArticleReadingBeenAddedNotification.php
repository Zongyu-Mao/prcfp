...
class EntryExtendedArticleReadingBeenAddedNotification extends Notification
{
    use Queueable;
     ...
     public function toDatabase($notifiable)
    {
        ...
        $link = '/publication/articleDetail/'.$extended->id.'/'.$extended->title.'#entryExtendedArticleReading';
        $doer = User::find($ex->creator_id);
        return [
            'creator_id'    => $doer->id,
            'creator'       => $doer->username,
            'link'          => $link,
            'matter'        => '你关注的著作《'.$extended->title.'》已经被词条《'.$entry->title.'》添加为延伸阅读。'
        ];
    }
