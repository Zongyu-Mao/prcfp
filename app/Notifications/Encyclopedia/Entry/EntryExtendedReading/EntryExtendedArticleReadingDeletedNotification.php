...
class EntryExtendedArticleReadingDeletedNotification extends Notification
{
    use Queueable;
     ...
     public function toDatabase($notifiable)
    {
        ...
        $link = '/encyclopedia/reading/'.$entry->id.'/'.$entry->title.'#entryExtendedArticleReading';
        // $doer = User::find($this->entryExtended->creator_id);
        return [
            'creator_id'    => auth('api')->user()->id,
            'creator'       => auth('api')->user()->username,
            'link'          => $link,
            'matter'        => '你关注的词条《'.$entry->title.'》的延伸阅读(著作)《'.$extended->title.'》已经删除。'
        ];
    }
