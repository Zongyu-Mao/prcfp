...
class ArticleContentCreatedNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        $classname = Classification::find($article->cid)->classname;
        $doer = User::find($this->articleContent->editor_id);
        return [
            'creator_id'    => $doer->id,
            'creator'       => $doer->username,
            'link'          => $link,
            'matter'        => '著作《'. $article->title .'》添加了第'. $this->articleContent->sort .'章节内容：<'. $this->articleContent->title .'>。'
        ];
    }
