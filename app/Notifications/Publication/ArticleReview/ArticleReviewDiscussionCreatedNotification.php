...
class ArticleReviewDiscussionCreatedNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        if($discussion->standpoint == '1'){
            $matter = '协作著作《'.$article->title.'》的评审计划<'.$articleReview->title.'>新增支持意见。';
        }elseif($discussion->standpoint == '3'){
            $matter = '协作著作《'.$article->title.'》的评审计划<'.$articleReview->title.'>新增中立意见。';
        }
        return [
