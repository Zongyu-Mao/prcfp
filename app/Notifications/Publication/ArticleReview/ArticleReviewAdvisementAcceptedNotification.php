...
class ArticleReviewAdvisementAcceptedNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        return [
            'creator_id'    => $this->articleReviewAdvise->recipient_id,
            'creator'       => $this->articleReviewAdvise->recipient,
            'link'          => $link,
            'matter'        => '你在著作《'.$article->title.'》的评审计划<'.$articleReview->title.'>中提出的建议<'.$this->articleReviewAdvise->title.'>已被接受。'
        ];
    }
