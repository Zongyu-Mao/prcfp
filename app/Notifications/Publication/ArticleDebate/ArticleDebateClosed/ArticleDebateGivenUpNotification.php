...
class ArticleDebateGivenUpNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        if($debate->status == '2'){
            $matter = '攻辩<'.$debate->title.'>由于攻方['.$debate->Aauthor.']放弃已经关闭。[来自著作：《'.$article->title.'》]。';
            $creator = $debate->Aauthor;
            $creator_id = $debate->Aauthor_id;
        }elseif($debate->status == '3'){
            $matter = '攻辩<'.$debate->title.'>由于辩方['.$debate->Bauthor.']放弃已经关闭。[来自著作：《'.$article->title.'》]。';
