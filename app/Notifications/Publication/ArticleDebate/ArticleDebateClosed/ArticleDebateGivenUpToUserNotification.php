...
class ArticleDebateGivenUpToUserNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        if($debate->status == '2'){
            $matter = '你作为辩方参与的攻辩<'.$debate->title.'>由于攻方['.$debate->Aauthor.']放弃已经关闭。[来自著作：《'.$article->title.'》]。';
        }elseif($debate->status == '3'){
            $matter = '你发起的攻辩<'.$debate->title.'>由于辩方['.$debate->Bauthor.']放弃已经关闭。[来自著作：《'.$article->title.'》]。';
        }
        return [
