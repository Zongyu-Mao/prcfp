...
class ArticleDebateTimeOutByRefereeClearedNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        if($debate->victory == '1'){
            $matter = '攻辩<'.$debate->title.'>已经结算。[裁判'.$debate->referee.']超时，自动结算。攻方['.$debate->Aauthor.']胜出。[来自著作：《'.$article->title.'》]。';
            $creator = $debate->Aauthor;
            $creator_id = $debate->Aauthor_id;
        }elseif($debate->victory == '2'){
            $matter = '攻辩<'.$debate->title.'>已经结算。[裁判'.$debate->referee.']超时，自动结算。攻方辩方['.$debate->Bauthor.']胜出。[来自著作：《'.$article->title.'》]。';
