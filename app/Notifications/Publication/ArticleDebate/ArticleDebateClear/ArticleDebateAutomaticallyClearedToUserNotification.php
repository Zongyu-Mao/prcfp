...
class ArticleDebateAutomaticallyClearedToUserNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        if($debate->victory == '1'){
            $matter = '你参与的攻辩<'.$debate->title.'>已经结算。攻方['.$debate->Aauthor.']胜出，感谢参与。[来自著作：《'.$article->title.'》]。';
            $creator = $debate->Aauthor;
            $creator_id = $debate->Aauthor_id;
        }elseif($debate->victory == '2'){
            $matter = '你参与的攻辩<'.$debate->title.'>已经结算。辩方['.$debate->Bauthor.']胜出，感谢参与。[来自著作：《'.$article->title.'》]。';
