...
class ArticleDebateAFDCreatedNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        $time = Carbon::parse($debate->AFDcreateTime)->addDays(5);
        // $doer = User::find($debate->Aauthor_id);
        return [
            'creator_id'    => $debate->Aauthor_id,
            'creator'       => $debate->Aauthor,
            'link'          => $link,
            'matter'        => '攻方['.$debate->Aauthor.']已经在攻辩：<'.$debate->title.'>发表攻方自由辩论。你可以在'.$time.'前回复。[来自著作：《'.$article->title.'》]。'
        ];
    }
