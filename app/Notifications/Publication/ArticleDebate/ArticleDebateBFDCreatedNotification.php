...
class ArticleDebateBFDCreatedNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        $time = Carbon::parse($debate->BFDcreateTime)->addDays(5);
        // $doer = User::find($debate->Aauthor_id);
        return [
            'creator_id'    => $debate->Bauthor_id,
            'creator'       => $debate->Bauthor,
            'link'          => $link,
            'matter'        => '对方['.$debate->Bauthor.']已经在你发起的攻辩：<'.$debate->title.'>发表辩方的自由辩论。你可以在'.$time.'前回复。[来自著作：《'.$article->title.'》]。'
        ];
    }
