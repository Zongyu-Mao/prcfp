...
class ExamResortSupportRejectedNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        $link = '/publication/resort/'.$exam->id.'/'.$exam->title.'#resort'.$this->examResort->id;
        return [
            'creator_id'    => $parentResort->author_id,
            'creator'       => $parentResort->author,
            'link'          => $link,
            'matter'        => '你在试卷《'.$exam->title.'》的求助内容<'.$parentResort->title.'>发布的帮助：<'.$this->examResort->title.'>已被拒绝。'
        ];
    }
    /**
