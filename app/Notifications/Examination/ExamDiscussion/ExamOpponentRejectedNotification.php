...
class ExamOpponentRejectedNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        $link = '/publication/discussion/'.$exam->id.'/'.$exam->title.'#opponent'.$this->examOpponent->id;
        return [
            'creator_id'    => $this->examOpponent->author_id,
            'creator'       => $this->examOpponent->author,
            'link'          => $link,
            'matter'        => '你关注（协作）的试卷《'.$exam->title.'》的[反对]立场讨论内容<'.$pOpponent.'>已被拒绝，理由：<'.$this->examOpponent->title.'>。'
        ];
    }
    /**
