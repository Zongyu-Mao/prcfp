...
class GroupDocCommentRepliedNotification extends Notification
{
    use Queueable;
    ...
    public function toDatabase($notifiable)
    {
        ...
        $link = '/home/organization/group/groupDocDetail/'.$doc->id.'/'.$doc->title.'#groupDocComment'.$comment->id;
        $doer = User::find($comment->author_id);
        return [
            'creator_id'    => $doer->id,
            'creator'       => $doer->username,
            'link'          => $link,
            'matter'        => '你发表的组织文档评论《'.$comment->title.'》已经回复。'
        ];
    }
