...
class CreateEntryDebatesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('entry_debates', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('cid');
            $table->integer('eid'); //词条id
            $table->enum('type',[0,1,2])->default(0); //来自：0协作1评审2讨论
            ...
            $table->enum('status',[0,1,2,3])->default(0); //状态：0正在辩论，1正常结束，2正方放弃，3辩方放弃
            $table->enum('victory',[1,2])->nullable(); //胜利方,1攻方2辩方
            $table->integer('views')->default(0);//浏览数
            $table->integer('heat')->default(0);//热度
            $table->text('remark');//辩方
            $table->timestamps();
        });
    }
    ...
    {
        Schema::dropIfExists('entry_debates');
    }
}
